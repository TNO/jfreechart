...
public abstract class ValueAxis extends Axis
	...
	/**
	 * Flag that indicates how the auto range should be aligned when it differs from
	 * the preferred range.
	 */
	...
	/**
	 * Sets the default auto-range-align and sends an {@link AxisChangeEvent} to all
	 * registered listeners.
	 * 
    ...
    public void setRange(Range range, boolean turnOffAutoRange, 
        ...
        if (rangeNewSize < this.rangeMinimumSize) {
        	if (!this.minimumRange || !this.range.contains(range)) {
            	// Only if we're not zoomed into maximum yet, we'll zoom in as much as possible
        		final double centralValue = range.getCentralValue();
        		final double halfLength = this.rangeMinimumSize / 2;
            	this.range = new Range(centralValue - halfLength, centralValue + halfLength);
	...
	public void setRangeMinimumSize(double rangeMinimumSize) {
		if (Double.isNaN(rangeMinimumSize) || rangeMinimumSize < MINIMUM_RANGE_MINIMUM_SIZE) {
			throw new IllegalArgumentException(
					"A positive range minimum size is required: " + rangeMinimumSize);
		}
	 ...
	 * necessary).
	 * 
	 * @author Yuri Blankenstein: Refactored to be able to query the auto-range of a
	 *         {@link ValueAxis}.
	 */
	...
	/**
	 * @return the calculated axis range to fit the range of values in the dataset.
	 *         <tt>null</tt> if range could not be calculated.
